package com.siro.gulimall.seckill.service.impl;
...
import com.siro.gulimall.seckill.fegin.CouponFeignService;
import com.siro.gulimall.seckill.fegin.ProductFeignService;
...
import com.siro.gulimall.seckill.vo.SeckillSessionWithSkusVo;
import com.siro.gulimall.seckill.vo.SkuInfoVo;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public List<SeckillSkuRedisTo> getCurrentSeckillSkus() {
        ...
        //从Redis中查询到所有key以seckill:sessions:开头的所有数据
        Set<String> keys = redisTemplate.keys(SeckillConstant.SESSIONS_CACHE_PREFIX + "*");
        for (String key : keys) {
            //seckill:sessions:1652508000000_1652522400000
            ...
            if (currentTime >= startTime && currentTime <= endTime) {
                //2、获取这个秒杀场次需要的所有商品信息 P318
                List<String> range = redisTemplate.opsForList().range(key, -100, 100);
                BoundHashOperations<String, String, String> hasOps = redisTemplate.boundHashOps(SeckillConstant.SKUKILL_CACHE_PREFIX);
                assert range != null;
    ...
    /**
     * TODO 待完成
     * 1、上架后续的流程，每一个数据都有过期时间。
     * 2、秒杀后续的流程，简化了收货地址等信息。
     *
     * @param killId
     * @param key
     * @param num
    ...
    private void saveSessionSkuInfo(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            //准备hash操作，绑定hash
            BoundHashOperations<String, Object, Object> operations = redisTemplate.boundHashOps(SeckillConstant.SKUKILL_CACHE_PREFIX);
